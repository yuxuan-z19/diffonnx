...
try:
    ref_model, _ = simplify(ref_program.model_proto)
    usr_model, _ = simplify(usr_program.model_proto)
except:
    cprint("🤕 ONNX simplification failed, using original models.", "magenta")
    ref_model = ref_program.model_proto
    usr_model = usr_program.model_proto

diff = OnnxDiff(ref_model, usr_model, verbose=True)
